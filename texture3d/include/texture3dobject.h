...
public:
	...
	Texture3dObject() 
	{
            ...
            std::cout << "Matrix type" << type2str(mapFile.type()) << std::endl;
            
            //cv::imwrite("somefiletotest.png", mapFile);
            
            //cv::Rect roi(1000, 1000,480,272);
            
            //cv::Mat croppedImage = mapFile(roi);
            
            //cv::imwrite("grayscaletest.bmp", croppedImage);
            ...
            imageCount = 100;
            imageHeight = mapFile.rows ;
            imageWidth = mapFile.cols;
                ...
                for (int ii = imageHeight - 1; ii >= 0; ii--)
                {
                    for (int jj = 0; jj < imageWidth; jj++)
                    {
                        ...
                        //std::cout << ii << " " << jj << " " <<  (int)mapFile.at<uchar>(jj, ii) << std::endl;
                         chBuffer[index] = mapFile.at<uchar>(ii, jj);
